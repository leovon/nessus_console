...
#coding:utf-8
#原作者:se55i0n
#原地址：https://github.com/se55i0n/Awvs_Nessus_Scanner_API/
#说明：原版本是python2环境，适用于nessus7环境。此版本通过修改少量代码，适配于python3环境Nessus10.0
...
class nessus(object):
	def __init__(self):
		...
		self._format = []
		#api地址
		self.server = 'https://127.0.0.1:8834'
		#api密钥
		self.secretKey = 'a8d10543313cefcb417a982d42ef3f8fdd987f157c51b02509aa4e507f58c87b'
		self.accessKey = '07f8799f8668cb600fa1dc574f83aaf65b9bb63a69b8f03456fe17dc9e37c71a'
	...
	def scan_type(self):
		...
		#原版本扫描策略顺序不对，这里重新做了对应
		#扫描策略选择
		try:
			i = 0
			for k in self.scan_rule:
				print(i,k)
				i += 1
			rule = input('[Nessus_Api/Set_Rule]>> ')
			if rule == '0':
	...
	def set_scan_name(self):
		#设置扫描任务名
		try:
			return input(self.O+'[Nessus_Api/Set_Scan_Name]>> '+self.W)
		except Exception as e:
			pass
	...
	def set_scan_target(self):
		#设置扫描目标
		try:
			return input(self.O+'[Nessus_Api/Set_Target]>> '+self.W)
		except Exception as e:
			pass
	...
	def check_id(self):
		...
		try:
			...
			if text != "":
				...
				task_id = input(self.O+'[Nessus_Api/Set_Task_Id]>> '+self.W)
		...
		try:
			...
			if text:
				for i in range(len(text)):
					...
					print(u'[-] 扫描目标: {}'.format(text[i]['hostname']))
					...
					print(u'[-] 信息提示: {}'.format(text[i]['info']))
		except Exception as e:
			pass
	...
	def set_format(self):
		try:
			report_format = {'Nessus':'nessus','HTML':'html','CSV':'csv','Nessus DB':'db','PDF':'pdf'}
			i = 0
			for k in report_format:
				print('[{}] '.format(i)+k)
				self._format.append(report_format[k])
				i += 1
			rule = input('[Nessus_Api/Set_Report_Format]>> ')
	...
	def handle(self):
		...
		try:
			...
			print('-'*48)
			while True:
				show = input(self.O+'[Nessus_Api]>> '+self.W)
				if show == 'view':
					self.view()
				elif show == 'scan':
					self.scan()
